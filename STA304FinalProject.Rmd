...
library(tableone)
table1 <- CreateTableOne(data = alldata)
...
p1 <- ggplot(rollingdata, aes(date) ) + 
  ...
  theme(plot.title = element_text(hjust = 0.5)) + labs(colour='Symptoms')
...
ggplot(rollingdata, aes(date) ) + 
  ...
  theme(plot.title = element_text(hjust = 0.5)) + labs(colour='Keywords') 
```
...
lm1tidy <- tidy(lm1)
kable(lm1tidy, caption = "Table 2: Multiple linear regression model using all 9 predictors")
...
kable(modtidy, caption = "Table 3: Reduced multiple linear regression model with 6 predictors")
...
huxtable::huxreg(propensity_score_regression) %>% set_caption("Table 4: Propensity Score Regression Analysis")
```
...
p3 <- ggplot(alldata, aes(date) ) + 
  ...
  theme(plot.title = element_text(hjust = 0.5)) + labs(colour='Type') 
